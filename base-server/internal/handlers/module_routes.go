...
// ModuleRouteRegistry handles registration of module routes for Swagger generation
type ModuleRouteRegistry struct{}

// RegisterDemoRoutes registers demo module routes with Swagger annotations
// This function exists solely for swag init to discover the routes and generate documentation
func (r *ModuleRouteRegistry) RegisterDemoRoutes(router *gin.RouterGroup, db *gorm.DB, cfg config.Config) {
	// These routes mirror what the demo module registers but with Swagger annotations
	// The actual routing is handled by the module system
