...
copy_dev_db() {
    ...
    # Check if dev containers are running (look in environments/dev-db directory)
    DEV_COMPOSE_FILE="../dev-db/docker-compose.yml"
    DEV_DB_CONTAINER=""
    ...
    if [ -f "$DEV_COMPOSE_FILE" ]; then
        ...
        if [ -z "$DEV_DB_CONTAINER" ] || ! docker compose -f $DEV_COMPOSE_FILE ps | grep -q "$DEV_DB_CONTAINER.*Up"; then
            log_error "Development database is not running in ../dev-db."
            log_info "Please start dev environment first: cd ../dev-db && docker compose up -d"
            exit 1
        fi
        log_success "Found running dev database: $DEV_DB_CONTAINER"
    else
        log_error "Development compose file not found: $DEV_COMPOSE_FILE"
        log_info "Please ensure you have ../dev-db/docker-compose.yml"
        exit 1
    fi
