...
func (s *CostProviderService) CreateCostProvider(req models.CreateCostProviderRequest, tenantID uint) (*models.CostProvider, error) {
...
func (s *CostProviderService) GetCostProviderByID(id, tenantID uint) (*models.CostProvider, error) {
...
func (s *CostProviderService) GetAllCostProviders(page, limit int, tenantID uint) ([]models.CostProvider, int64, error) {
	var costProviders []models.CostProvider
	var total int64

	offset := (page - 1) * limit

	// Count total records for the tenant
	if err := s.db.Model(&models.CostProvider{}).Where("tenant_id = ?", tenantID).Count(&total).Error; err != nil {
...
func (s *CostProviderService) UpdateCostProvider(id, tenantID uint, req models.UpdateCostProviderRequest) (*models.CostProvider, error) {
	var costProvider models.CostProvider
	if err := s.db.Where("id = ? AND tenant_id = ?", id, tenantID).First(&costProvider).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, fmt.Errorf("cost provider with ID %d not found", id)
...
func (s *CostProviderService) SearchCostProviders(query string, page, limit int, tenantID uint) ([]models.CostProvider, int64, error) {
	var costProviders []models.CostProvider
	var total int64

	// Build search query for the tenant
	searchPattern := "%" + query + "%"
	searchQuery := s.db.Model(&models.CostProvider{}).Where(
